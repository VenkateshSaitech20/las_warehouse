...
class EditUserRoleMaster extends StatefulWidget {
  final UserRole? userRoleData;
  const EditUserRoleMaster({super.key, this.userRoleData});

  @override
  State<EditUserRoleMaster> createState() => _AddUserRoleMasterState();
}

class _AddUserRoleMasterState extends State<EditUserRoleMaster> {
  static final GlobalKey<FormState> _editUserRoleFormKey =
      GlobalKey<FormState>();
  ...
  @override
  void initState() {
    _userRoleController.text = widget.userRoleData!.userRole!;
    privilegeMenus = userRoleController.userRoleMenusList.value;
    if (widget.userRoleData!.menus!.hasRead != null &&
        widget.userRoleData!.menus!.hasRead!.isNotEmpty) {
      selectedPrivilegeMenus = [];
      for (int j = 0; j < privilegeMenus.length; j++) {
        for (int ij = 0;
            ij < widget.userRoleData!.menus!.hasRead!.length;
            ij++) {
          if (privilegeMenus[j].menuId ==
              widget.userRoleData!.menus!.hasRead![ij]) {
  ...
  void _trySubmitForm() async {
    ...
    if (_editUserRoleFormKey.currentState!.validate()) {
